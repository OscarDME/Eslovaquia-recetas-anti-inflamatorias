    ...
    <section className="w-full bg-[#fffdf7] py-14 md:py-20 px-4">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-center gap-3 mb-10">
          <HelpCircle className="w-8 h-8 text-[#0B533E]" />
          <h2 className="text-[#0B533E] text-2xl md:text-4xl font-black">
            {faq.headline}
          </h2>
        </div>
          ...
          {faq.items.map((item, index) => (
            <div
              ...
              className={`rounded-xl overflow-hidden transition-all duration-300 ${
                openIndex === index 
                  ? "bg-white shadow-lg border border-[#0B533E]/10" 
            ...
            >
              ...
              >
                <span className={`text-sm md:text-base font-medium pr-4 transition-colors ${
                  openIndex === index ? "text-[#0B533E] font-semibold" : "text-gray-800"
                }`}>
                  {item.question}
                </span>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 ${
                  openIndex === index 
                    ? "bg-[#0B533E] text-white rotate-180" 
